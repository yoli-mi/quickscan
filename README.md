...
常驻系统托盘的二维码扫码器：按快捷键呼出，**截屏框选**屏幕上任意二维码——网页里的码、聊天窗口的码、图片里的码——立即识别，一键复制或打开。**不借助手机、不打开浏览器、几乎不占资源。**

## 使用

1. 按 `Ctrl+Alt+Q`（可自定义）呼出小部件
2. 点击「截屏扫码」，屏幕进入框选模式
3. 拖拽框选二维码 → 自动识别
...
- 苹果风格动效：呼出从托盘区弹入，收起滑回托盘区，互为镜像
- 独立设置页：自动复制、提示音、窗口置顶、开机自启动、自动收起
...
> For users in mainland China, configuring the rsproxy.cn cargo mirror is recommended; keep rustup in an ASCII-only path.
